...
class EmployeeDao:
    ...
    async def find_least_loaded_employee(self, db: AsyncSession, task: TaskSchema):
        free_employee_query = select(self.model).filter(self.model.tasks is None)
        result = await db.execute(free_employee_query)
        free_employee = result.unique().scalars().first()

        least_loaded_employee_query = select(
            self.model, count(self.model.tasks).label('count')
        ).outerjoin(self.model.tasks).group_by(self.model.id).order_by(asc('count'))
        result = await db.execute(least_loaded_employee_query)
        least_loaded_employee = result.unique().scalars().first()

        optimal_employee_query = select(
            self.model
        ).join(self.model.tasks).filter(
            self.model.tasks.any(parent_task_id=task.parent_task_id, status='active')
